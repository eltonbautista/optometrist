  ...
  <header class="mb-2 px-4 shadow">
    <div class="relative mx-auto flex max-w-screen-lg flex-col py-4 sm:flex-row sm:items-center sm:justify-between">
      <a class="flex items-center text-2xl font-black" href="/">
        <span class="mr-2 text-3xl text-blue-600"><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img"
      ...
      <nav aria-label="Header Navigation" class="peer-checked:block hidden pl-2 py-6 sm:block sm:py-0">
        <ul v-if="navbarLinks" class="flex flex-col gap-y-4 sm:flex-row sm:gap-x-8">
          <li v-for="(text, index) in navbarLinks" :key="index + text">
            <NavLink :link="textToLink(text)">{{ text }}</NavLink>
          </li>
        </ul>
      </nav>
    </div>
...
export default {
  ...
  data() {
    return {
      navbarLinks: ['home', 'about', 'services', 'contact', 'forms', 'crypto currencies']
    };
  },
  methods: {
    textToLink(text: string): string {
      if (text.length > 0) {
